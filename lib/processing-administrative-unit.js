...
export async function createAdministrativeUnitdRelationships(administrativeUnitdUuid) {
  ...
  if (administrativeUnit) {
    ...
    } else if (!isWorshipAdministrativeUnit(administrativeUnit.classification)) {
...
async function createGoverningBodies(administrativeUnit) {
  const governingBodyClassifications = await getGoverningBodyClassifications(administrativeUnit);
...
async function createGoverningBodiesInTime(administrativeUnit, governingBodies) {
  let startDate;
  let endDate;
  ...
  for (let governingBody of governingBodies) {
    const governingBodyInTime = await createGoverningBodyInTime(governingBody.uri, startDate, endDate);
...
async function createMandates(governingBodiesInfo) {
  ...
  governingBodiesInfo.forEach(governingBodyInfo => {
    ...
    const mandatesClassifications = getMandateClassifications(governingBody.classification);
    ...
    if (mandatesClassifications) {
      mandatesClassifications.forEach(mandateClassification => {
        const mandateWithSameClassification = mandatesToCreate.find(mandate => mandate.classification == mandateClassification);
        
        if (mandateWithSameClassification) {
          mandateWithSameClassification.governingBodiesInTime.push(governingBodyInTime);
        } else {
  ...
  for (let mandateToCreate of mandatesToCreate) {
    await createMandate(mandateToCreate.classification, mandateToCreate.governingBodiesInTime);
  }
}

async function createBestuursfuncties(governingBodiesInfo) {
  for (let governingBodyInfo of governingBodiesInfo) {
    const bestuursfunctieClassifications = getBestuursfunctieClassifications(governingBodyInfo.governingBody.classification);

    if (bestuursfunctieClassifications) {
      for (let bestuursfunctieClassification of bestuursfunctieClassifications) {
        await createBestuursfunctie(bestuursfunctieClassification, governingBodyInfo.governingBodyInTime);
      }
    }
  }
}
...
async function createMinisters(administrativeUnit) {
  const ministerClassifications = MINISTER_CLASSIFICATIONS[administrativeUnit.religionType];

  for (let ministerClassification of ministerClassifications) {
    await createMinister(ministerClassification, administrativeUnit.uri);
  }
}
...
function isWorshipAdministrativeUnit(classification) {
  return classification == ADMIN_UNIT_CLASSIFICATIONS.EB || classification == ADMIN_UNIT_CLASSIFICATIONS.CB;
}
...
function getGoverningBodyClassifications(administrativeUnit) {
  let governingBodyClassifications = GOVERNING_BODY_CLASSIFICATIONS[administrativeUnit.classification];

  if ( isWorshipAdministrativeUnit(administrativeUnit.classification)) {
    governingBodyClassifications = governingBodyClassifications[administrativeUnit.religionType];
  }
...
function getMandateClassifications(governingBodyClassification) {
  let mandateClassifications = MANDATE_CLASSIFICATIONS[governingBodyClassification];

  return mandateClassifications;
}

function getBestuursfunctieClassifications(governingBodyClassification) {
  let bestuursfunctieClassifications = BESTUURSFUNCTIE_CLASSIFICATIONS[governingBodyClassification];
