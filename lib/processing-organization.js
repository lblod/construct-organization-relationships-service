...
import {getInOprichtingWorshipGoverningBodyPeriod, getUpcomingWorshipGoverningBodyPeriods} from "./utils";
...
export async function createOrganizationRelationships(organizationUuid) {
  ...
  if (isWorshipServiceInOprichting) {
    // - Worship service "In Oprichting": single open-ended period
    periods = getInOprichtingWorshipGoverningBodyPeriod();
 ...
 *     governing bodies.
 * @param {string} [startDate] - start date for the first period.
 */
export async function createPeriodsForExistingGoverningBodies(
  governingBodies,
  startDate
) {
  const periods = getUpcomingWorshipGoverningBodyPeriods();
  if (startDate) {
