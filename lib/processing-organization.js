...
import {getUpcomingWorshipGoverningBodyPeriods, getApplicableHelftMandate} from "./utils";
...
export async function createOrganizationRelationships(organizationUuid) {
  ...
  const applicableHelftMandate = getApplicableHelftMandate(periods[0]);
 ...
 * @param {string} [applicableHelftMandate] - the URI of the "grote helft" or
 ...
 */
async function createMandates(governingBodiesInfo, isWorshipServiceInOprichting = false, applicableHelftMandate = null) {
  let mandatesToCreate = [];
  const spanningMandateClassifications = applicableHelftMandate
    ? [...MANDATES_SPANNING_ALL_PERIODS, applicableHelftMandate]
    : MANDATES_SPANNING_ALL_PERIODS;
...
export async function createPeriodsForExistingGoverningBodies(
  ...
  for (const period of periods) {
    ...
    await createMandates(governingBodiesInfo, false, applicableHelftMandate);
  }
}
