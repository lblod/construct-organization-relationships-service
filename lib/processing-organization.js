import {
  ...
  getExistingGoverningBodyInTime,
  createMandate,
  getExistingMandate,
  linkMandateToGoverningBodyInTime,
...
import {
  ...
  START_DATE_POLITIECOLLEGE,
  MANDATE_CLASSIFICATIONS,
  BESTUURSFUNCTIE_CLASSIFICATIONS,
  MINISTER_CLASSIFICATIONS,
  IN_OPRICHTING_STATUS,
  WORSHIP_IN_OPRICHTING_MANDATES,
  MANDATES_SPANNING_ALL_PERIODS,
} from "./config";
import {getUpcomingWorshipGoverningBodyPeriods, getApplicableHalfMandate} from "./utils";
...
export async function createOrganizationRelationships(organizationUuid) {
  ...
  const isWorshipServiceInOprichting = isWorshipService(organization.classification) &&
                         organization.status === IN_OPRICHTING_STATUS;

  let periods;
  if (isWorshipServiceInOprichting) {
    // - Worship service "In Oprichting": single open-ended period starting today
    periods = [{
      startDate: new Date(),
      endDate: undefined,
    }];
  } else if (isWorshipAdministrativeUnit(organization.classification)) {
    // - Other worship orgs: both current and next 3-year period
    periods = getUpcomingWorshipGoverningBodyPeriods();
  } else if (organization.classification === ORGANIZATION_CLASSIFICATIONS.POLITIEZONE) {
    periods = [{
      startDate: START_DATE_POLITIECOLLEGE,
      endDate: undefined,
    }];
  } else {
    // - Non-worship orgs: single open-ended period from 2019
    periods = [{
  ...
  const applicableHalfMandate = getApplicableHalfMandate(periods[0]);
  ...
  for (const period of periods) {
    ...
    await createMandates(
      governingBodiesInfo,
      isWorshipServiceInOprichting,
        applicableHalfMandate,
    );
...
async function createGoverningBodiesInTime(governingBodies, period) {
  ...
  for (let governingBody of governingBodies) {
    let governingBodyInTime = await getExistingGoverningBodyInTime(
      governingBody.uri,
      period.startDate,
      period.endDate,
    );

    if (!governingBodyInTime) {
      governingBodyInTime = await createGoverningBodyInTime(
        governingBody.uri,
        period.startDate,
        period.endDate,
      );
      console.log(
        `Created new governing body in time for period ${period.startDate} - ${period.endDate || 'open'}`
      );
    } else {
      console.log(
        `Reusing existing governing body in time for period ${period.startDate} - ${period.endDate || 'open'}`
      );
    }
 ...
 * @param {boolean} isWorshipServiceInOprichting - if true, only create mandates for worship
 *     services in "In Oprichting" status (voorzitter, secretaris, penningmeester).
 * @param {string} [applicableHalfMandate] - the URI of the "grote helft" or
 *     "kleine helft" mandate classification that should be shared across this
 *     pair of periods, if any. The other helft classification is still
 *     created, but gets its own independent mandate per period.
 */
async function createMandates(governingBodiesInfo, isWorshipServiceInOprichting = false, applicableHalfMandate = null) {
  let mandatesToCreate = [];
  const spanningMandateClassifications = applicableHalfMandate
    ? [...MANDATES_SPANNING_ALL_PERIODS, applicableHalfMandate]
    : MANDATES_SPANNING_ALL_PERIODS;

  for (const governingBodyInfo of governingBodiesInfo) {
    const governingBody = governingBodyInfo.governingBody;
    const governingBodyInTime = governingBodyInfo.governingBodyInTime;

    let mandatesClassifications = getMandateClassifications(
      governingBody.classification,
    );

    if (isWorshipServiceInOprichting && mandatesClassifications) {
      mandatesClassifications = mandatesClassifications.filter(
        (classification) => WORSHIP_IN_OPRICHTING_MANDATES.includes(classification)
      );
    }
    ...
    if (mandatesClassifications) {
      for (const mandateClassification of mandatesClassifications) {
        if (spanningMandateClassifications.includes(mandateClassification)) {
          const existingMandate = await getExistingMandate(
            mandateClassification,
            governingBody.uri,
          );

          if (existingMandate) {
            await linkMandateToGoverningBodyInTime(
              existingMandate,
              governingBodyInTime,
            );
          } else {
            await createMandate(mandateClassification, [governingBodyInTime]);
          }
        } else {
...
export function isWorshipService(classification) {
...
/**
 * Create time-specialized governing bodies and mandates for existing governing
 * bodies. This is used for status transitions where the governing bodies already
 * exist but we need to create new periods.
 * @param {Array<{uri: string, classification: string}>} governingBodies - existing
 *     governing bodies.
 * @param {string} startDate - start date for the first period, normally the
 *     transition date.
 */
export async function createPeriodsForExistingGoverningBodies(
  governingBodies,
  startDate
) {
  const periods = getUpcomingWorshipGoverningBodyPeriods(new Date(startDate));
  periods[0].startDate = startDate;
  ...
  for (const period of periods) {
    ...
    await createMandates(governingBodiesInfo, false, applicableHalfMandate);
  }
}
