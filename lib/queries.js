...
/**
 * Check if there are any open-ended bestuursorganen in time for a given organization.
 * Open-ended means no mandaat:bindingEinde is set.
 * @param {string} organizationUri - the URI of the organization.
 * @returns {Promise<boolean>} True if there are open-ended bestuursorganen, false otherwise.
 */
export async function hasOpenEndedBestuursorganenInTime(organizationUri) {
  // prettier-ignore
  const result = await query(`
    ${PREFIXES}
...
/**
 * Get an existing governing body in time for the given period, if it exists.
 * @param {string} governingBodyUri - the URI of the governing body.
 * @param {string} startDate - the start date of the period.
 * @param {string} [endDate] - the optional end date of the period.
 * @returns {Promise<string|null>} The URI of the existing governing body in time, or null if not found.
 */
export async function getExistingGoverningBodyInTime(governingBodyUri, startDate, endDate) {
  // prettier-ignore
  ...
  const result = await query(`
    ...
    SELECT ?governingBodyInTime WHERE {
      ?governingBodyInTime generiek:isTijdspecialisatieVan ${sparqlEscapeUri(governingBodyUri)} ;
        mandaat:bindingStart ${sparqlEscapeDateTime(startDate)} .
  ...
  if (result.results.bindings.length) {
    return result.results.bindings[0].governingBodyInTime.value;
  }
...
/**
 * Get an existing mandate with the given role for a governing body, if one
 * exists.
 * @param {string} role - the URI of the role of the mandate.
 * @param {string} governingBodyUri - the URI of the governing body.
 * @returns {Promise<string|null>} The URI of the existing mandate, or null if
 *     none was found.
 */
export async function getExistingMandate(role, governingBodyUri) {
  // prettier-ignore
  const result = await query(`
    ${PREFIXES}
    ...
    SELECT ?mandate WHERE {
      ?governingBodyInTime generiek:isTijdspecialisatieVan ${sparqlEscapeUri(governingBodyUri)} ;
        org:hasPost ?mandate .
      ?mandate org:role ${sparqlEscapeUri(role)} .
    }
    LIMIT 1
  `);
...
/**
 * Link an existing mandate to an additional time-specialised governing body.
 * @param {string} mandateUri - the URI of the mandate.
 * @param {string} governingBodyInTime - the URI of the time-specialised
 *     governing body to link.
 * @returns {Promise<void>}
 */
export async function linkMandateToGoverningBodyInTime(mandateUri, governingBodyInTime) {
  // prettier-ignore
  await update(`
    ${PREFIXES}

    INSERT DATA {
      ${sparqlEscapeUri(governingBodyInTime)} org:hasPost ${sparqlEscapeUri(mandateUri)} .
    }
  `);
}
