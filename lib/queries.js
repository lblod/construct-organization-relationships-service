...
export async function createGoverningBody(administrativeUnit, classification) {
  ...
  if (result.results.bindings.length) {
    ...
    INSERT {
      ...
      BIND(CONCAT(${sparqlEscapeString(classificationLabel)}, " ", ?administrativeUnitLabel) as ?governingBodyLabel)
    }
  `);

    return uri;
  } else {
    throw Error(`classificationLabel not found ${classification}`);
  }
}
